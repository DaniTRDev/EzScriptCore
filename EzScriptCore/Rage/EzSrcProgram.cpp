...
namespace Ez
{
	...
	EzSrcProgram::EzSrcProgram() : m_Header(nullptr), m_RSC7Offset(0), m_StringTableOffsets(nullptr),
	    ...
	    m_ScriptBuff(std::make_unique<EzBuffer>())
	{
	}
	...
	EzScriptStatus EzSrcProgram::ParseHeader(std::istream& Stream, std::string ExpectedScriptName)
	{
		...
		m_ScriptBuff->FromStream(Stream);

		std::int32_t Magic = 0;
		m_ScriptBuff->Read((char*)&Magic, sizeof(Magic));

		if (Magic == 0x37435352) /*don't know lmfao*/
		{
			m_RSC7Offset = 0x10;
			m_ScriptVersion = rage::RageResourceVersion::RRV_7;
		}
		else if (Magic == 0x405a9ed0) /*1.58 an backward*/
			m_ScriptVersion = rage::RageResourceVersion::RRV_MinusThan10;
		else /*1.59 and forward*/
			m_ScriptVersion = rage::RageResourceVersion::RRV_10;
